    ...
    <div className="h-[calc(100vh-8rem)] flex flex-col bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="flex border-b border-slate-100">
        <button 
          ...
          className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'health' ? 'bg-primary-50 text-primary-600 border-b-2 border-primary-500' : 'text-slate-600 hover:bg-slate-50'}`}
        >
          <Bot size={18} /> Health Assistant
        </button>
        <button 
          onClick={() => setActiveTab('medicine')}
          className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'medicine' ? 'bg-primary-50 text-primary-600 border-b-2 border-primary-500' : 'text-slate-600 hover:bg-slate-50'}`}
        >
          <Pill size={18} /> Medicine Info Bot
        </button>
      </div>
        ...
        {filteredMessages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex max-w-[85%] gap-3 ${msg.type === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.type === 'user' ? 'bg-slate-200' : 'bg-primary-100 text-primary-600'}`}>
                {msg.type === 'user' ? <User size={16} /> : (activeTab === 'health' ? <Bot size={16} /> : <Pill size={16} />)}
              </div>
              <div className={`p-4 rounded-2xl text-sm ${msg.type === 'user' ? 'bg-primary-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-700 rounded-tl-none shadow-sm'}`}>
      ...
      <div className="p-4 bg-white border-t border-slate-100">
        <div className="flex gap-2">
          <input
            ...
            placeholder={activeTab === 'health' ? "Ask about your symptoms..." : "Enter medicine name (e.g., aspirin, metformin)..."}
            disabled={loading}
            className={`flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          />
